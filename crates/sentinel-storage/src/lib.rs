...
impl StorageManager {
    ...
    pub async fn new(
        config: &crate::sqlite::SqliteConfig,
        duckdb_config: &crate::duckdb::DuckDbConfig,
    ) -> anyhow::Result<Self> {
...
impl EventRepository for DuckDbEventRepository {
    ...
    async fn query(
        &self,
        query: sentinel_core::traits::EventQuery,
    ) -> CoreResult<Arc<dyn sentinel_core::traits::EventCursor>> {
        let cursor = self
            .duckdb
            .query_events(query)
            .await
    ...
    async fn aggregate(
        &self,
        agg: sentinel_core::traits::AggregationQuery,
    ) -> CoreResult<sentinel_core::traits::AggregationResult> {
        self.duckdb
