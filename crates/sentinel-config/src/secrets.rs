...
use age::{x25519, Decryptor, Encryptor, Identity};
...
impl SecretsManager {
    ...
    pub fn new() -> Self {
        Self { identity: None, recipient: None, secrets_path: None }
    }
    ...
    pub fn with_key(identity_str: &str) -> Result<Self> {
        ...
        Ok(Self { identity: Some(identity), recipient: Some(recipient), secrets_path: None })
    }
    ...
    pub fn decrypt(&self, ciphertext: &str) -> Result<String> {
        ...
        // Decode from base64
        let encrypted = base64::decode(ciphertext).context("Failed to decode base64")?;

        let decryptor =
            Decryptor::new(encrypted.as_slice()).context("Failed to create decryptor")?;
    ...
    pub async fn save_secrets_file<P: AsRef<Path>>(
        ...
        let content = toml::to_string(&encrypted_secrets).context("Failed to serialize secrets")?;

        fs::write(&path, content).await.with_context(|| {
            format!("Failed to write secrets file: {}", path.as_ref().display())
        })?;
